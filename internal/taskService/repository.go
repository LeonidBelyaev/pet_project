...
type TaskRepository interface {
	CreateTask(userID uint, task Message) (Message, error)
...
func (r *taskRepository) CreateTask(userID uint, task Message) (Message, error) {
	if userID == 0 {
		return Message{}, errors.New("userID is required")
	}
	task.UserID = userID
